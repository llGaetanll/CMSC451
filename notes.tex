...
Align(x, y):
  ...
  weight, you can't simply consider ``one more edge". Let's look at an example
  ...
  \begin{itemize}
    ...
    \item {\bf Yes}: Then $\opt(i, v) = \min_{u \in V}\big(\opt(i - 1, u) +
      ...
      Here we consider the optimal way to get to a neighbor of $v$, and then
      look for the best way to get from that neighbor to $u$.
  \end{itemize}
