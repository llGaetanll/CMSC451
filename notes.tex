...
MaxFlow($G$, $C$, $s$, $t$)
  ...
  \Lemma {
    The optimal value for the load balancing problem is at least

    \[
      \max \left\{ \frac{1}{m} \sum_i t_i, \,\, \max_i (t_i) \right\}
    \]

    The first value is the average load per machine, the latter is the total
  ...
  \Lemma {
    If there are more than $m$ jobs, then $T^* \ge 2t_{m + 1}$.

    Here we order the jobs so $t_1 \ge t_2 \ge \cdots \ge t_n$.
  }
  {
    Some machine gets $2$ jobs, and $t_i$ is the last assigned, then %each of which have duration at least $t_{m + 1}$.
...
GreedyCover($S$, $r$):
  ...
  \sidenote {
    The Final exam is on Wednesday, May 17, 10:30am to 12:30pm in SPP 0301.
  }
...
GreedyCover($S$):
  ...
  \Theorem {
    \texttt{GreedyCover($S$)} returns a set of $k$ points with covering radius
    at most twice the optimal radius.
  }
  {
    ...
    \note {
      We don't know the optimal radius! But the point is that it exists.
      Here, we suppose it's more than twice the best radius.
    }
  ...
  \note {
    Often, greedy strategies can do quite well for approximation algorithms!
    Although sometimes, this might not work.
  }
  ...
  \example {
    For a fair coin, $\Omega = \{H, T\}$, the probability of $H$ is $P(H) = P(T)
    = \frac{1}{2}$
  }

  \example {
    For $n$ fair coins, $\Omega = \{H, T\}^n$, then $P(i) = \frac{1}{2^n}$, the
    probability of $k$ heads in $n$ flips is $\frac{1}{2^n} \binom{n}{k}$.
  }
  ...
  \example {
    For $n$ fair coins, the number of heads is a random variable.
  }
  ...
  \example {
    The expected number of heads for $n$ fair coins should be $n / 2$, but let's
    see if we get that from the definition of expected value.

    Let $\Omega = \{H, T\}^n$, then

    \begin{align*}
      E(H) =&\sum_{i \in \Omega} \frac{1}{2^n} \cdot (\text{number of heads in $i$}) \\
           =&\sum_{k = 0}^n \frac{1}{2^n} \binom{n}{k} k \\
           =&\frac{1}{2}
    \end{align*}
  }
  ...
  \example {
    Let's look at what we mean by {\it contracting} an edge.

    Take a look at this graph.

    \TODO{}

    If we wanted to contract the $2$-$4$ edge, this is what our resulting graph
    would look like.

    \TODO{}

    If we wanted to now contract either edge between $2, 4$-$5$, our resulting
    graph would look like this

    \TODO{}

    So the number of edges can decrease by {\it more than one} each time!
  }

  \note {
    If we produce multiple edges between two vertices, those edges are going to
    be more likely to be get contracted!
  }
  ...
  \Lemma {
    The contraction algorithm returns a global minimum cut with probability at
    least $\frac{1}{\binom{n}{2}}$.

    This isn't great, but it's a bound! The idea here is that you don't need to
    run this process only once. If you run this multiple times, you're actually going
    to be pretty likely to have found the true minimal cut.
  }
  {
